    ...
    FILE *openFile(const char *fileName, bool append)
    {
        ...
        { // dump to file
            ...
            {                                 // append the file
                ...
                if (!file)
                { // if append failed, create a new file
                    file = fopen(fileName, "wt");
                }
            }
            ...
            while (metaDataBucketLinkedListElement != NULL)
            {
                ...
                if (isVerbose())
                {
                    fprintf(m_Dumper, "FreeAllMem: freeing 0x%p (size %d[bytes]) allocated in %s:%d\n",
                            actualPointerInRecord, (int)metaDataBucketLinkedListElement->size,
...
void __mem_leak_check(size_t &memLeakCount, uint64_t &memLeakSize, bool verbose, const char *fileDumperName,
                      bool append)
{
